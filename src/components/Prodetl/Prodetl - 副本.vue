<template>
	<div class="prodetl">
		...
		<div class="prodetl_top">
			...
			<div class="prodetl_top9">
				<img src="../../assets/naba.png" class="prodetl_top10">
				<div class="prodetl_top11">支持开团并邀请{{detail.tuan_num}}人参团，人数不足自动退款</div>
			</div>
				...
				<div>
					<div class="prodetl_con6" v-for="item in group_list">
						...
						<div class="prodetl_con11">
							<div class="prodetl_con9">还差{{item.group_status}}人成团</div>
							<div class="prodetl_con10">剩余{{item.djs}}</div>
						</div>
						<div class="prodetl_con12" @click="cantuanFn(item.face,item.nickname,item.group_status,item.end_time,item.group_id)">去参团</div>
					</div>
		...
		<transition name="fade">
			<div class="pindantan" v-if="pindanflag">
				...
				<div class="pindantan7">
					<div class="pindantan8" @click="pintuanpay">参与拼团</div>
				</div>
			</div>
		</transition>
		<div :class="danduflag == true ? 'dandugou zhan' : 'dandugou'">
			...
			<div class="dandugou9">
				<div v-for="(item,index) in details.spec_items" :class="active == index ? 'dandugou10 on' : 'dandugou10'" @click="jinxuanFn(index,item.spec_id)">{{item.spec_name}}</div>
...
export default {
	...
	computed:{
		details1(){
			...
			let group_lists=this.details.group_lists;
	...
	methods:{
		...
		cantuanFn: function(a,b,c,d,e){
			...
			that.tuan_userimg = a;
			that.tuan_user = b;
			that.tuan_usernum = that.detail.tuan_num - c;
			setInterval( ()=> {
		...
		danduFn: function(e){
			...
			if(that.details.spec_items != ''){
				...
				var product_id = that.product_id;
				var shopnum = that.nums;
				that.$router.push('/orderpayinf?product_id='+ product_id + '&shopnum=' + shopnum);
			}
		},
		...
		payforFn: function(){
			...
			that.$router.push('/orderpayinf?product_id='+ product_id + '&shoptype=' + shoptype + '&shopguige=' + shopguige + '&shopnum=' + shopnum + '&group_id=' + group_id);
		}
	},
}
</script>
...
<style scoped>
	...
	.prodetl_top2{width: 100%;height: 285px;}
	...
	.prodetl_con5{width: auto;height: 12px;}
	...
	.pindantan{position: fixed;top: 30%;left: 10%;right: 10%;background: #fff;z-index: 3;border-radius: 6px;text-align: center;}
	...
	.morepin{position: fixed;top: 18%;left: 10%;right: 10%;background: #fff;border-radius: 6px;z-index: 3;}
